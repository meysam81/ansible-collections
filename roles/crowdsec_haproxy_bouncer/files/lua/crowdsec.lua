...
local function NewTemplate(path)
    ...
    if content == nil then
        error("failed to read template file: " .. tostring(path))
    end
...
local function init()
    ...
    local ok, err = pcall(function() runtime.ban = NewTemplate(BAN_TEMPLATE_PATH) end)
    if not ok then
        runtime.logger.error("Failed to load ban template: " .. tostring(err))
        return
    end
    ...
    local ok2, err2 = pcall(function() runtime.captcha = NewTemplate(CAPTCHA_TEMPLATE_PATH) end)
    if not ok2 then
        runtime.logger.error("Failed to load captcha template: " .. tostring(err2))
        return
    end
